...
const Navbar: NextPage<INavbarProps> = ({
  ...
  const [contentfulNavbarData, setContentfulNavbarData] =
    useState<NavbarModule.INavbar>();
  ...
  const windowWidthTablet: number = +tabletSizeWindow.tabletSizeWindow.slice(
    0,
    3,
  );
  ...
  const navbarData: NavbarModule.INavbar = new Map(
    Object.entries(navbarSectionProps),
  )
    .values()
    .next().value;

  const navlinksData: NavbarModule.INavlink[] = new Map(
    Object.entries(navbarSectionProps),
  )
    .values()
    .next().value['navlinks'];
  ...
  const renderNavlinks: JSX.Element[] = navlinksData.map(
    (navlinkInfo: NavbarModule.INavlink, index: number) => {
      //console.log("navlinkInfo: ", navlinkInfo)
      return (
        <li
          className={`${navlinkInfo.fields.name}-title`}
          key={index}
        >
          <Link
          ...
          >
            <a
              ...
              className={`underline ${
                activeNavLink === index ? 'active' : 'not-active'
              }`}
            >
  ...
  const handleMenuToggle = useCallback(
    () => setMobileNavOpen((mobileNavOpen: boolean) => !mobileNavOpen),
    [],
  );
  ...
  return (
    <header
      className={`header-navbar-active ${isNavbarActive ? 'hidden' : ''} ${
        scrollYPosition > 0 ? 'shadow' : ''
      }`}
    >
      <Logo
      ...
      <MobileMenu
        ...
        socialMediaContact={socialMediaData}
        isMobileMenuOpen={mobileNavOpen}
      />
      <DesktopMenu renderNavlinks={renderNavlinks} />
      {/* BEGIN MODAL */}
      {isModalActive ? (
        <Modal
