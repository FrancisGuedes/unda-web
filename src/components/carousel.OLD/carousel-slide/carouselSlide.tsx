...
import { CarouselModule } from '../../../lib/interfaces/contentful/ihero';
import { StayWithUsImageModule } from '../../../lib/interfaces/contentful/isolutions';
import { concatHttpsAndUrlFromContentful } from '../../../utils/utility';
import {
  LabelCarouselClassName,
  LabelCarouselSlideTextClassName,
} from '../carousel';
import { ReviewModule } from '../../../lib/interfaces/contentful/ilayout';
...
const CarouselSlide = ({
  ...
  const [toggleReviewCommentStatus, setToggleReviewCommentStatus] = useState<
    boolean[]
  >([]);
  ...
  function slideFileUrl(
    ...
    slidesMedia.forEach((slide: any, index: number) => {
      const slideUrl: CarouselModule.IFile =
        slide['fields']['media']['fields']['file'];
  ...
  const renderSlides: JSX.Element[] = slides.map(
    (slide: any, index: number) => {
      const mediaByIndex = (index: number) =>
        slideFile[index % slideFile.length];
      if (isSlideImageActive) {
        // Render Image Slides
        const slideDescription: CarouselModule.IFields3 =
        ...
        let reviewCardDescription: ReviewModule.IContent2 =
          reviewCardFields['theReview']['content'][0]['content'][0];
  ...
  return (
    <>
      <div className={className?.embla_container}>{renderSlides}</div>
    </>
  );
};
