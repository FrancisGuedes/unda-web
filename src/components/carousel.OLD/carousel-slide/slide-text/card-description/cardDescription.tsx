...
const CardDescription = ({
  ...
  const labelRel: LabelButton = {
    ...functionalityAlias.component.carousel.button,
  };
  const labelButtonName: LabelButtonName = {
    ...functionalityAlias.component.carousel.cardReviewButtonName,
  };
  ...
  const validateTextLength = (
    ...
    return text.length <= numberOfCharacters
      ? text
      : text.substring(0, numberOfCharacters).concat('...');
  };

  return (
    <>
      <div className={classNameTextSlide?.description} key={index}>
        {toggleReviewCommentStatus[index] &&
        ...
        {reviewCardDescription.value.length > 185 ? (
          ...
          >
            {toggleReviewCommentStatus[index]
              ? labelButtonName.less
              : labelButtonName.more}
          </AppButton>
        ) : null}
      </div>
    </>
